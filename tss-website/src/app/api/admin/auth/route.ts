...
export async function POST(req: NextRequest) {
  ...
  try {
   ...
   const allowedUser = (process.env.ADMIN_CONSOLE_USER || "TwoStepsStudioAdmin").trim().toLowerCase();
     if (!secret) {
      return NextResponse.json({ error: "Brak konfiguracji has豉" }, { status: 500 });
    }
    if (!password || !name) {
      return NextResponse.json({ error: "Nieprawid這we dane" }, { status: 400 });
    }
   if (name.trim().toLowerCase() !== allowedUser) {
      return NextResponse.json({ error: "Nieprawid這wa nazwa" }, { status: 401 });
    }
     if (password !== secret) {
      return NextResponse.json({ error: "Has這 nieprawid這we" }, { status: 401 });
    }
    ...
    return NextResponse.json({ error: "B章d serwera" }, { status: 500 });
  }
}
