...
export default function SettingsPage() {
  ...
  const AppearanceOption = ({ value, icon: Icon, label }: { value: "light" | "dark" | "system"; icon: any; label: string }) => (
    <button
      ...
      className={`group relative flex flex-col items-center justify-center gap-2 p-5 rounded-2xl border transition-all hover:shadow-lg ${
        appearance === value ? "bg-[var(--color-general)]/10 border-[var(--color-general)]" : "bg-white/70 dark:bg-white/5 border-[var(--border-color)]"
      }`}
    >
  ...
  const ColorChip = ({ value, label, className }: { value: "ocean" | "crimson" | "emerald" | "violet" | "amber"; label: string; className: string }) => (
    <button
      ...
      className={`flex items-center gap-3 px-5 py-4 rounded-2xl border transition-all hover:shadow-lg ${colorTheme === value ? "bg-[var(--color-general)]/10 border-[var(--color-general)]" : "bg-white/70 dark:bg-white/5 border-[var(--border-color)]"} ${className}`}
    >
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] glass p-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mt-2 font-[family-name:var(--font-outfit)]">{t.settings.subtitle}</p>
          </div>
        ...
        <Card className="rounded-[2.5rem] glass">
          ...
          <CardContent className="grid grid-cols-2 gap-4">
            <button
              ...
              className={`flex items-center gap-3 px-5 py-6 rounded-2xl border transition-all ${language === "pl" ? "bg-[var(--color-general)]/10 border-[var(--color-general)]" : "bg-white/70 dark:bg-white/5 border-[var(--border-color)]"}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-3 px-5 py-6 rounded-2xl border transition-all ${language === "en" ? "bg-[var(--color-general)]/10 border-[var(--color-general)]" : "bg-white/70 dark:bg-white/5 border-[var(--border-color)]"}`}
            >
              <Languages className="text-[var(--color-general)]" />
              <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-10 items-start">
        <Card className="rounded-[2.5rem] glass">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between px-4 py-3 rounded-2xl bg-white/70 dark:bg-white/5 border">
        ...
        <Card className="rounded-[2.5rem] glass lg:col-span-2">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="rounded-3xl bg-white/70 dark:bg-white/5 border p-6 space-y-2">
