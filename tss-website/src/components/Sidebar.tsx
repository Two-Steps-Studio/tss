    ...
    <div className="px-4 mb-6">
      <div className="glass rounded-[2rem] p-5 overflow-hidden relative group border border-white/5">
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[11px] font-black uppercase tracking-[0.2em] opacity-40 flex items-center gap-2">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-xs font-bold opacity-60">{t.nav.online}</span>
            </div>
            <span className="text-xs font-black">{(stats.online_users || 0).toLocaleString()}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-xs font-bold opacity-60">Społeczność</span>
            </div>
            <span className="text-xs font-black">{(stats.total_members || 0).toLocaleString()}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-xs font-bold opacity-60">Wiadomości</span>
            </div>
            <span className="text-xs font-black">{(stats.messages_today || 0).toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
...
export function Sidebar() {
  ...
  const handleMouseEnter = (id: string) => {
    ...
    }, 450); // Zwiększone opóźnienie - otwiera się dopiero po dłuższym przytrzymaniu kursora
  };
  ...
  const handleMouseLeave = () => {
    ...
    }, 100); // Zmniejszone opóźnienie - zamyka się szybko
  };
  ...
  const SidebarContent = (
    <div className="flex flex-col h-full py-4">
      ...
      <div className="px-6 mb-8 flex items-center justify-center relative">
        <Link href="/" className="block relative group">
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
            className="relative z-10 w-[120px] h-[68px] mx-auto"
          >
            {mounted && (
              <Image
                src={logo}
                ...
                width={120}
                height={68}
                className="transition-opacity duration-500 object-contain h-[68px] opacity-100"
                unoptimized
              />
            )}
          </motion.div>
          <div className="absolute inset-0 bg-[var(--color-general)] opacity-0 group-hover:opacity-10 blur-2xl transition-opacity rounded-full" />
        </Link>
        <button
          onClick={close}
      ...
      <nav className="flex-1 px-4 overflow-y-auto no-scrollbar pb-6" suppressHydrationWarning>
        <h2 className="text-[11px] font-black uppercase tracking-[0.2em] mb-4 px-4 opacity-30">{t.nav.mainMenu}</h2>
          ...
          {mounted && (
            <>
              <Button
                ...
                className="flex-1 h-10 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 transition-all border border-black/10 dark:border-white/10"
              >
              ...
              <Button
                ...
                className="flex-1 h-10 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 transition-all border border-black/10 dark:border-white/10 font-black text-[10px]"
              >
