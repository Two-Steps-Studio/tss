...
const MAX_ATTACHMENT_SIZE_MB = 5 * 1024 * 1024;

/** Render image thumbnail or icon placeholder if no attachment exists for this task. */
export function renderAttachmentThumbnail(url?: string, alt?: string): JSX.Element {
  return (
    <Image 
      src={url || ""} 
      width="64" 
      height="64" 
      alt={alt || ""} 
      className="rounded-sm w-[10vw] sm:w-32 max-w-full object-contain mt-2" 
    />
  );
}
